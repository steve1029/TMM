%% Field visualization case 2 right-left
...
%% .    input field - eigenmode of right waveguide

% excitation eigenmode
Einc=zeros(2*L,1);
laynt=Nlay; % right waveguide
...
Lay_cof2(1:2*L,1)=CR2;
Lay_cof2(2*L+1:4*L)=Einc;
...
%       DEt3(k,l)=abs(abs(ET3x_cof(k,l))^2+abs(ET3y_cof(k,l))^2+abs(ET3z_cof(k,l))^2)*real(kz_tra(k,l)/(kfz));
%       
%    end;   
% end;
%    
