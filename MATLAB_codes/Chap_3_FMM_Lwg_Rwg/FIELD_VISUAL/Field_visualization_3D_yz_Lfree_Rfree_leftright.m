...
for znt=1:zn
 ...
 % region 2
  ...
  Ey_yz2=zeros(length(yy),sum(rg2cnt));
  Ex_yz2=zeros(length(yy),sum(rg2cnt));
  Ez_yz2=zeros(length(yy),sum(rg2cnt));
  Hy_yz2=zeros(length(yy),sum(rg2cnt));
  Hx_yz2=zeros(length(yy),sum(rg2cnt));
  Hz_yz2=zeros(length(yy),sum(rg2cnt));     

  % region 3
  
  Ey_yz3=zeros(length(yy),rg3cnt);
  Ex_yz3=zeros(length(yy),rg3cnt);
  Ez_yz3=zeros(length(yy),rg3cnt);
  Hy_yz3=zeros(length(yy),rg3cnt);
  Hx_yz3=zeros(length(yy),rg3cnt);
  Hz_yz3=zeros(length(yy),rg3cnt);     
...
for k=1:NBx
   for l=1:NBy
      ...
      ET3z_cof(k,l)=-( kx_vc(k)*ET3x_cof(k,l)+ky_vc(l)*ET3y_cof(k,l) )/kz_vc(k,l); 
      HT3y_cof(k,l)=HT3_y((k-1)*NBy+l);
      HT3x_cof(k,l)=HT3_x((k-1)*NBy+l );
      HT3z_cof(k,l)=-( kx_vc(k)*HT3x_cof(k,l)+ky_vc(l)*HT3y_cof(k,l) )/kz_vc(k,l); 
...
for k=1:NBx
   for l=1:NBy
      ...
      DEt3(k,l)=abs(abs(ET3x_cof(k,l))^2+abs(ET3y_cof(k,l))^2+abs(ET3z_cof(k,l))^2)*real(kz_vc(k,l)/(kz0));
...
[z3 y3]=meshgrid(zz(zz3ind(1):zz3ind(2)),yy); % region 3
    for k=1:NBx
         for l=1:NBy
           Ey_yz3=Ey_yz3+ET3y_cof(k,l)*exp(j*kz_vc(k,l)*(z3-ac_thick(Nlay))).*exp(j*( kx_vc(k)*x+ky_vc(l)*y3 ));   
           Ex_yz3=Ex_yz3+ET3x_cof(k,l)*exp(j*kz_vc(k,l)*(z3-ac_thick(Nlay))).*exp(j*( kx_vc(k)*x+ky_vc(l)*y3 ));
           Ez_yz3=Ez_yz3+ET3z_cof(k,l)*exp(j*kz_vc(k,l)*(z3-ac_thick(Nlay))).*exp(j*( kx_vc(k)*x+ky_vc(l)*y3 ));
           Hy_yz3=Hy_yz3+HT3y_cof(k,l)*exp(j*kz_vc(k,l)*(z3-ac_thick(Nlay))).*exp(j*( kx_vc(k)*x+ky_vc(l)*y3 ));   
           Hx_yz3=Hx_yz3+HT3x_cof(k,l)*exp(j*kz_vc(k,l)*(z3-ac_thick(Nlay))).*exp(j*( kx_vc(k)*x+ky_vc(l)*y3 ));
           Hz_yz3=Hz_yz3+HT3z_cof(k,l)*exp(j*kz_vc(k,l)*(z3-ac_thick(Nlay))).*exp(j*( kx_vc(k)*x+ky_vc(l)*y3 ));

            end;
        end;
        
%         Ey_yz3=Ey_yz3+Uy*exp(j*( kx0*x+ky0*y3-kz0*(z3-ac_thick(Nlay))));
%         Ex_yz3=Ex_yz3+Ux*exp(j*( kx0*x+ky0*y3-kz0*(z3-ac_thick(Nlay))));
%         Ez_yz3=Ez_yz3+Uz*exp(j*( kx0*x+ky0*y3-kz0*(z3-ac_thick(Nlay))));
%         Hy_yz3=Hy_yz3+Vy*exp(j*( kx0*x+ky0*y3-kz0*(z3-ac_thick(Nlay))));
%         Hx_yz3=Hx_yz3+Vx*exp(j*( kx0*x+ky0*y3-kz0*(z3-ac_thick(Nlay))));
%         Hz_yz3=Hz_yz3+Vz*exp(j*( kx0*x+ky0*y3-kz0*(z3-ac_thick(Nlay))));
    ...
    mfHz=Mf_Hz(:,:,laynt)*tmp2;
     ...
     for k=1:NBx
      for l=1:NBy
          ...
          Ex_yz2(:,zz_cnt+fc_ind(laynt,1)-1)=Ex_yz2(:,zz_cnt+fc_ind(laynt,1)-1)+( pfEx((k-1)*NBy+l) + mfEx((k-1)*NBy+l) )*exp(j*( kx_vc(k)*x+ky_vc(l)*yy));   % Ex_xz    
          Ey_yz2(:,zz_cnt+fc_ind(laynt,1)-1)=Ey_yz2(:,zz_cnt+fc_ind(laynt,1)-1)+( pfEy((k-1)*NBy+l) + mfEy((k-1)*NBy+l) )*exp(j*( kx_vc(k)*x+ky_vc(l)*yy));   % Ey_xz
          Ez_yz2(:,zz_cnt+fc_ind(laynt,1)-1)=Ez_yz2(:,zz_cnt+fc_ind(laynt,1)-1)+( pfEz((k-1)*NBy+l) + mfEz((k-1)*NBy+l) )*exp(j*( kx_vc(k)*x+ky_vc(l)*yy));   % Ex_xz    
          
          Hx_yz2(:,zz_cnt+fc_ind(laynt,1)-1)=Hx_yz2(:,zz_cnt+fc_ind(laynt,1)-1)+( pfHx((k-1)*NBy+l) + mfHx((k-1)*NBy+l) )*exp(j*( kx_vc(k)*x+ky_vc(l)*yy));   % Hx_xz
          Hy_yz2(:,zz_cnt+fc_ind(laynt,1)-1)=Hy_yz2(:,zz_cnt+fc_ind(laynt,1)-1)+( pfHy((k-1)*NBy+l) + mfHy((k-1)*NBy+l) )*exp(j*( kx_vc(k)*x+ky_vc(l)*yy));   % Hy_xz
          Hz_yz2(:,zz_cnt+fc_ind(laynt,1)-1)=Hz_yz2(:,zz_cnt+fc_ind(laynt,1)-1)+( pfHz((k-1)*NBy+l) + mfHz((k-1)*NBy+l) )*exp(j*( kx_vc(k)*x+ky_vc(l)*yy));   % Hx_xz
...
Ex_yz=[Ex_yz1 Ex_yz2  Ex_yz3];
Ey_yz=[Ey_yz1 Ey_yz2  Ey_yz3];
Ez_yz=[Ez_yz1 Ez_yz2  Ez_yz3];

Hx_yz=[Hx_yz1 Hx_yz2  Hx_yz3];
Hy_yz=[Hy_yz1 Hy_yz2  Hy_yz3];
Hz_yz=[Hz_yz1 Hz_yz2  Hz_yz3];


figure(1); imagesc(zz/um,yy/um,real(Ex_yz)); set(gca,'fontsize',16); colorbar; title('Ex-yz'); xlabel('z-axis(um)');ylabel('y-axis(um)');colormap(hot); set(gca,'fontsize',16);
figure(2); imagesc(zz/um,yy/um,real(Ey_yz)); set(gca,'fontsize',16); colorbar; title('Ey-yz'); xlabel('z-axis(um)');ylabel('y-axis(um)');colormap(hot); set(gca,'fontsize',16);
figure(3); imagesc(zz/um,yy/um,real(Ez_yz)); set(gca,'fontsize',16); colorbar; title('Ez-yz'); xlabel('z-axis(um)');ylabel('y-axis(um)');colormap(hot); set(gca,'fontsize',16);


figure(4); imagesc(zz/um,yy/um,real(Hx_yz)); set(gca,'fontsize',16); colorbar; title('Hx-yz'); xlabel('z-axis(um)');ylabel('y-axis(um)'); colormap(hot); set(gca,'fontsize',16);
figure(5); imagesc(zz/um,yy/um,real(Hy_yz)); set(gca,'fontsize',16); colorbar; title('Hy-yz'); xlabel('z-axis(um)');ylabel('y-axis(um)'); colormap(hot); set(gca,'fontsize',16);
figure(6); imagesc(zz/um,yy/um,real(Hz_yz)); set(gca,'fontsize',16); colorbar; title('Hz-yz'); xlabel('z-axis(um)');ylabel('y-axis(um)'); colormap(hot); set(gca,'fontsize',16);
