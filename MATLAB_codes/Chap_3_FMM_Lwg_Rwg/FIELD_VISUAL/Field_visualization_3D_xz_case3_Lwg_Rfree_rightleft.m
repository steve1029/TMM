%% Field visualization case 3 right-left
...
[z2 x2]=meshgrid(zz(zz2ind(1):zz2ind(2)),xx); % region 2
    for k=1:NBx
        ...
        Ez_xz2=Ez_xz2+Uz*exp(j*( kix*x2+kiy*y-kz0*(z2-ac_thick(Nlay))));
        Hy_xz2=Hy_xz2+Vy*exp(j*( kix*x2+kiy*y-kz0*(z2-ac_thick(Nlay))));
        Hx_xz2=Hx_xz2+Vx*exp(j*( kix*x2+kiy*y-kz0*(z2-ac_thick(Nlay))));
        Hz_xz2=Hz_xz2+Vz*exp(j*( kix*x2+kiy*y-kz0*(z2-ac_thick(Nlay))));
