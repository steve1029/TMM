...
% freespace1 - grating region - freespace1
...
%    for l=1:NBy
%       
%       DEt1(k,l)=abs(abs(T1x_cof(k,l))^2+abs(T1y_cof(k,l))^2+abs(T1z_cof(k,l))^2)*real(Akz_vc(k,l)/(kz0));
%       DEt2(k,l)=abs(abs(T2x_cof(k,l))^2+abs(T2y_cof(k,l))^2+abs(T2z_cof(k,l))^2)*real(Akz_vc(k,l)/(kz0));
%       
%    end;   
% end;
%    
...
figure(4); imagesc((real([0*Ey_xz1 AcaGEy_xz 0*Ey_xz2]*(exp(-i*kk)))));  caxis([-3 3]);
pause(0.1);
end;
