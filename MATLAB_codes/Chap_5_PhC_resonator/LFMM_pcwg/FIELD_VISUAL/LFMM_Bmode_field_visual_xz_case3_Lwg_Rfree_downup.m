...
ET4_temp=BLT34*D;      %  positive coupling coefficient
...
for laynt=1:SNlay
   for pbm_ind=1:pcnt
            ...
            bGEy_xz(:,:,laynt)=bGEy_xz(:,:,laynt)+bPG_Ey_xz(:,:,inc_mode)*exp(-bp_evalue(inc_mode)*bTx*(SNlay-laynt+1)); %Sx
            bGEx_xz(:,:,laynt)=bGEx_xz(:,:,laynt)+bPG_Ex_xz(:,:,inc_mode)*exp(-bp_evalue(inc_mode)*bTx*(SNlay-laynt+1)); %Sx
         	bGEz_xz(:,:,laynt)=bGEz_xz(:,:,laynt)+bPG_Ez_xz(:,:,inc_mode)*exp(-bp_evalue(inc_mode)*bTx*(SNlay-laynt+1)); 
            
            bGHy_xz(:,:,laynt)=bGHy_xz(:,:,laynt)+bPG_Hy_xz(:,:,inc_mode)*exp(-bp_evalue(inc_mode)*bTx*(SNlay-laynt+1)); %Sx
            bGHx_xz(:,:,laynt)=bGHx_xz(:,:,laynt)+bPG_Hx_xz(:,:,inc_mode)*exp(-bp_evalue(inc_mode)*bTx*(SNlay-laynt+1)); %Sx
         	bGHz_xz(:,:,laynt)=bGHz_xz(:,:,laynt)+bPG_Hz_xz(:,:,inc_mode)*exp(-bp_evalue(inc_mode)*bTx*(SNlay-laynt+1)); 
