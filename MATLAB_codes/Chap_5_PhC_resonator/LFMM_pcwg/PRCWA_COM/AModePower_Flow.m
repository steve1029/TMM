...
%%% power calculation 
aPw=zeros(2*L,1); % z-direction power
aNw=zeros(2*L,1); % z-direction power
% positive_mode
for pbm_ind=1:2*L
    
        tt=PS*ap_evector(:,pbm_ind);
        ...
        aPw(pbm_ind)=0.5*real( sum(efx.*conj(hfy)-efy.*conj(hfx)) );
...
for pbm_ind=1:2*L
       tt=PS*am_evector(:,pbm_ind);
        ...
        aNw(pbm_ind)=0.5*real( sum(efx.*conj(hfy)-efy.*conj(hfx)) );
...
figure(1); plot(abs(aPw));title('A_positive_mode power');
figure(2); plot(abs(aNw));title('A_negative_mode power');
