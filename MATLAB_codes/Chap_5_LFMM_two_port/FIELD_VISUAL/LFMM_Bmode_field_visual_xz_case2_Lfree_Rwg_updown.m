...
for laynt=1:SNlay
   for pbm_ind=1:pcnt
         ...
         % incidence mode
            ...
            bGEy_xz(:,:,laynt)=bGEy_xz(:,:,laynt)+bNG_Ey_xz(:,:,inc_mode)*exp(bm_evalue(inc_mode)*bTx*(laynt)); %Sx
            bGEx_xz(:,:,laynt)=bGEx_xz(:,:,laynt)+bNG_Ex_xz(:,:,inc_mode)*exp(bm_evalue(inc_mode)*bTx*(laynt)); %Sx
         	bGEz_xz(:,:,laynt)=bGEz_xz(:,:,laynt)+bNG_Ez_xz(:,:,inc_mode)*exp(bm_evalue(inc_mode)*bTx*(laynt)); 
            
            bGHy_xz(:,:,laynt)=bGHy_xz(:,:,laynt)+bNG_Hy_xz(:,:,inc_mode)*exp(bm_evalue(inc_mode)*bTx*(laynt)); %Sx
            bGHx_xz(:,:,laynt)=bGHx_xz(:,:,laynt)+bNG_Hx_xz(:,:,inc_mode)*exp(bm_evalue(inc_mode)*bTx*(laynt)); %Sx
         	bGHz_xz(:,:,laynt)=bGHz_xz(:,:,laynt)+bNG_Hz_xz(:,:,inc_mode)*exp(bm_evalue(inc_mode)*bTx*(laynt)); 
