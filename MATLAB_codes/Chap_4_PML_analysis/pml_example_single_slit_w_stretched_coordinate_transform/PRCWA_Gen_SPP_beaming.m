...
% Layer structure ------------------------------------------
Nlay=3;% old or new pml. free space - slit - grating(thick zero) - pml_space
% Nlay=1; % no pml  free space - wg1 - free space
...
lay_thick(1)=0; % this value should not be referred to. Since laynt=1 indicates semi-infinite  input region
lay_thick(2)=300*nm;    %   dielectric grating
lay_thick(3)=0; % this value should not be referred to. Since laynt=3 indicates semi-infinite output region
...
%     bu_zz(:,:,lnt)=  bu_L(:,:,lnt);
    ...
    % new pml
    eps_xx(:,:,lnt)=eps_L_xx(:,:,lnt);
    eps_yy(:,:,lnt)=eps_L_yy(:,:,lnt);
    eps_zz(:,:,lnt)=eps_L_zz(:,:,lnt);
    aps_xx(:,:,lnt)=aps_L_xx(:,:,lnt);
    aps_yy(:,:,lnt)=aps_L_yy(:,:,lnt);
    aps_zz(:,:,lnt)=aps_L_zz(:,:,lnt);
    mu_xx(:,:,lnt)=  mu_L_xx(:,:,lnt);
    mu_yy(:,:,lnt)=  mu_L_yy(:,:,lnt);
    mu_zz(:,:,lnt)=  mu_L_zz(:,:,lnt);
    bu_xx(:,:,lnt)=  bu_L_xx(:,:,lnt);
    bu_yy(:,:,lnt)=  bu_L_yy(:,:,lnt);
    bu_zz(:,:,lnt)=  bu_L_zz(:,:,lnt);
