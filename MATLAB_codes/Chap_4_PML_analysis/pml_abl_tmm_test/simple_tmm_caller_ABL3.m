...
for param_idx=1:param_len
    ...
    mode        = 1; % TM
    ep          = [1 1+(p1*ll).^p3+i*(p2*ll).^p4 1].^2;
%     mu          = [1 1 1];
    mu=ones(1,LL+2);
%     d           = [0 1 0]*lambda;N=length(ep);
    d=[0 1/LL*ones(1,LL) 0]*5*lambda;N=length(ep);
...
figure(31);set(gca,'fontsize',16);set(gca,'fontname','times new roman');hold on;grid on;box on;
...
figure(32);set(gca,'fontsize',16);set(gca,'fontname','times new roman');hold on;grid on;box on;
...
figure(33);set(gca,'fontsize',16);set(gca,'fontname','times new roman');hold on;grid on;box on;
