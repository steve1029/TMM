...
% Final boundary matching
 ...
 %%%
    ...
    inv2a1b=inv(I-R_temp2a*R_temp1b);
    inv1b2a=inv(I-R_temp1b*R_temp2a);
...
%     TTb=T_temp1b*inv(I-R_temp2a*R_temp1b)*T_temp2b;
    RRa=(R_temp1a+T_temp1b*inv2a1b*R_temp2a*T_temp1a);
 	TTa=T_temp2a*inv1b2a*T_temp1a;

 	RRb=(R_temp2b+T_temp2a*inv1b2a*R_temp1b*T_temp2b);
    TTb=T_temp1b*inv2a1b*T_temp2b;
   
    for k=1:Nlay
%     tCa(:,:,k)=Ca(:,:,k)*inv(I-R_temp1b*R_temp2a)*T_temp1a;
%     tCb(:,:,k)=Cb(:,:,k)+Ca(:,:,k)*inv(I-R_temp1b*R_temp2a)*R_temp1b*T_temp2b;
    tCa(:,:,k)=Ca(:,:,k)*inv1b2a*T_temp1a;
    tCb(:,:,k)=Cb(:,:,k)+Ca(:,:,k)*inv1b2a*R_temp1b*T_temp2b;
  	end;
    Ca=tCa;
    Cb=tCb;
...
%     tCb(:,:,k)=Cb(:,:,k)*inv(I-R_temp2a*R_temp1b)*T_temp2b;
    tCa(:,:,k)=Ca(:,:,k)+Cb(:,:,k)*inv2a1b*R_temp2a*T_temp1a;
    tCb(:,:,k)=Cb(:,:,k)*inv2a1b*T_temp2b;
    end; % for k
    Ca=tCa;
    Cb=tCb;
