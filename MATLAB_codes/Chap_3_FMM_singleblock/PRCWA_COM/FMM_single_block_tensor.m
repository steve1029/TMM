...
% When the material is fully anisotropic, FMM_single_block_tensor.m is
% used. The objective of this function is to solve the eigenvalue equation
% of off-diagonal full anisotropic materials.

% codes in lines 1~143 does the preparation to construct the eigenvalue
% equation of Eq. (4.1.6a). For single layer structures, z-dependent term
% is not included in the structure representations, so Gz is zero in Eq.
% (3.2.7).

% In Eq. (4.1.6), Kx and Ky are implemented as Kx and Ky in MATLAB code
% lines 26-34. The system matrix (4.1.6) is implemented by St. This part is
% described in code lines 177~250. In the same way as the case of diagonal
% anisotropy material. Lalanne¡¯s experience rule of use of reciprocal 
% permittivity and permeability is employed in the implementation. 
% The Maxwell eigenvalue equation (4.1.6) is numerically solved and the 
% resulting eigenvalues and eigenvectors are saved to W and Dt, respectively. 
%This parti is in lines 252~253.

% The eigenmode classification rule is implemented at lines 165~195 in
% FMM_single_block.m and at lines 256~280 in FMM_single_block_tensor.m

% All variables necessary in Eq. (4.1.22a) are prepared. Eq. (4.1.22a) is
% implemented at lines 304~305 by S.
