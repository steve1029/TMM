%% Field visualization case 1 left-right

xx=[-Tx/2:Tx*0.002:Tx/2]';
z_start=-2*um;
z_end=7*um;
z_inc=50*nano;
...
HT2_x=HT2_temp(L+1:2*L);
      ...
      ET2y_cof=ET2_y;
      ET2x_cof=ET2_x;
      ET2z_cof=-( kx_tra*ET2x_cof+ky_tra*ET2y_cof )/kz_tra; 
      HT2y_cof=HT2_y;
      HT2x_cof=HT2_x;
      HT2z_cof=-( kx_tra*HT2x_cof+ky_tra*HT2y_cof )/kz_tra; 

      ET1y_cof=ET1_y;
      ET1x_cof=ET1_x;
	  ET1z_cof=( kx_ref*ET1x_cof+ky_ref*ET1y_cof )/kz_ref; 
      HT1y_cof=HT1_y;
      HT1x_cof=HT1_x;
	  HT1z_cof=( kx_ref*HT1x_cof+ky_ref*HT1y_cof )/kz_ref; 
   

% diffraction efficiency
      
      DEt1=abs(abs(ET1x_cof)^2+abs(ET1y_cof)^2+abs(ET1z_cof)^2)*real(kz_ref/(kz_ref));
      DEt2=abs(abs(ET2x_cof)^2+abs(ET2y_cof)^2+abs(ET2z_cof)^2)*real(kz_tra/(kz_ref));
...
[z1 x1]=meshgrid(zz(zz1ind(1):zz1ind(2)),xx); % region 1
           ...
           Ey_xz1=Ey_xz1+ET1y_cof*exp(j*( kx_ref*x1+ky_ref*y-kz_ref*z1 ));   
           Ex_xz1=Ex_xz1+ET1x_cof*exp(j*( kx_ref*x1+ky_ref*y-kz_ref*z1 ));
           Ez_xz1=Ez_xz1+ET1z_cof*exp(j*( kx_ref*x1+ky_ref*y-kz_ref*z1 ));
           
           Hy_xz1=Hy_xz1+HT1y_cof*exp(j*( kx_ref*x1+ky_ref*y-kz_ref*z1 ));   
           Hx_xz1=Hx_xz1+HT1x_cof*exp(j*( kx_ref*x1+ky_ref*y-kz_ref*z1 ));
           Hz_xz1=Hz_xz1+HT1z_cof*exp(j*( kx_ref*x1+ky_ref*y-kz_ref*z1 ));
...
[z2 x2]=meshgrid(zz(zz2ind(1):zz2ind(2)),xx); % region 2
           ...
           Ey_xz2=Ey_xz2+ET2y_cof*exp(j*kz_tra*(z2-ac_thick(Nlay))).*exp(j*( kx_tra*x2+ky_tra*y ));   
           Ex_xz2=Ex_xz2+ET2x_cof*exp(j*kz_tra*(z2-ac_thick(Nlay))).*exp(j*( kx_tra*x2+ky_tra*y ));
           Ez_xz2=Ez_xz2+ET2z_cof*exp(j*kz_tra*(z2-ac_thick(Nlay))).*exp(j*( kx_tra*x2+ky_tra*y ));
           Hy_xz2=Hy_xz2+HT2y_cof*exp(j*kz_tra*(z2-ac_thick(Nlay))).*exp(j*( kx_tra*x2+ky_tra*y ));   
           Hx_xz2=Hx_xz2+HT2x_cof*exp(j*kz_tra*(z2-ac_thick(Nlay))).*exp(j*( kx_tra*x2+ky_tra*y ));
           Hz_xz2=Hz_xz2+HT2z_cof*exp(j*kz_tra*(z2-ac_thick(Nlay))).*exp(j*( kx_tra*x2+ky_tra*y ));

       
        
%         Ey_xz2=Ey_xz2+Uy*exp(j*( kx0*x2+ky0*y-kz0*(z2-ac_thick(Nlay))));
%         Ex_xz2=Ex_xz2+Ux*exp(j*( kx0*x2+ky0*y-kz0*(z2-ac_thick(Nlay))));
%         Ez_xz2=Ez_xz2+Uz*exp(j*( kx0*x2+ky0*y-kz0*(z2-ac_thick(Nlay))));
%         Hy_xz2=Hy_xz2+Vy*exp(j*( kx0*x2+ky0*y-kz0*(z2-ac_thick(Nlay))));
%         Hx_xz2=Hx_xz2+Vx*exp(j*( kx0*x2+ky0*y-kz0*(z2-ac_thick(Nlay))));
%         Hz_xz2=Hz_xz2+Vz*exp(j*( kx0*x2+ky0*y-kz0*(z2-ac_thick(Nlay))));
...
zp=ac_thick(laynt);
  ...
  for zz_cnt=1:length(zzp)
    ...
    mfHz=Mf_Hz(:,:,laynt)*tmp2;
          ...
          GEx_xz(:,zz_cnt+fc_ind(laynt,1)-1)=GEx_xz(:,zz_cnt+fc_ind(laynt,1)-1)+( pfEx + mfEx )*exp(j*( kx_vc*xx+ky_vc*y));   % Ex_xz    
          GEy_xz(:,zz_cnt+fc_ind(laynt,1)-1)=GEy_xz(:,zz_cnt+fc_ind(laynt,1)-1)+( pfEy + mfEy )*exp(j*( kx_vc*xx+ky_vc*y));   % Ey_xz
          GEz_xz(:,zz_cnt+fc_ind(laynt,1)-1)=GEz_xz(:,zz_cnt+fc_ind(laynt,1)-1)+( pfEz + mfEz )*exp(j*( kx_vc*xx+ky_vc*y));   % Ex_xz    
          
          GHx_xz(:,zz_cnt+fc_ind(laynt,1)-1)=GHx_xz(:,zz_cnt+fc_ind(laynt,1)-1)+( pfHx + mfHx )*exp(j*( kx_vc*xx+ky_vc*y));   % Hx_xz
          GHy_xz(:,zz_cnt+fc_ind(laynt,1)-1)=GHy_xz(:,zz_cnt+fc_ind(laynt,1)-1)+( pfHy + mfHy )*exp(j*( kx_vc*xx+ky_vc*y));   % Hy_xz
          GHz_xz(:,zz_cnt+fc_ind(laynt,1)-1)=GHz_xz(:,zz_cnt+fc_ind(laynt,1)-1)+( pfHz + mfHz )*exp(j*( kx_vc*xx+ky_vc*y));   % Hx_xz
